...
<div id="valida_descripcion" class="alert alert-danger" style="display:none"></div>
	...
	<form method="POST" action="<?php echo e(route('nueva_campana.store')); ?>" id="universo_manual" enctype="multipart/form-data">
			...
			<div class="col-md-4" style="padding:0px 4px 0px 0px">
				<div class="cajas_altura_paneles">
					...
					<div class="col-md-12">
						...
						<input type="text" id="asunto" class="input_box_text_panel" name="asunto" value="<?php echo e($request->asunto); ?>" style="width:95%" oninvalid="setCustomValidity('Completar este campo es obligatorio')" oninput="setCustomValidity('')" required>
						<?php else: ?>
					...
					<div class="col-md-12">
					<?php if(isset($request->mensaje)): ?>
						<textarea name="comentario" id="mensaje" rows="10" cols="40" style="resize:none; width:95%; height:130px;border:none;" placeholder="Escribe tu comentario...." oninvalid="setCustomValidity('Completar este campo es obligatorio')" oninput="setCustomValidity('')" required><?php echo e($request->mensaje); ?></textarea>
					<?php else: ?>
					...
					<div class="col-md-6" style="width:130px">
						...
						<select id="select_tipo" name="tipo_campania" class="select_box_body" oninvalid="setCustomValidity('Completar este campo es obligatorio')" oninput="setCustomValidity('')" required>
							...
							<?php if(isset($request->tipo_campania)): ?>
					...
					<div id="automatica_div" class="ocultar_tipo">
						...
						<div class="col-md-6" >
							...
							<select id="select_proceso" name="proceso" class="select_box_body" oninvalid="setCustomValidity('Completar este campo es obligatorio')" oninput="setCustomValidity('')" >
								<option value="">seleccionar...</option>
								<?php if(isset($proceso_actual)): ?>
									<option id="<?php echo e($proceso_actual->id); ?>" value="<?php echo e($proceso_actual->proceso); ?>" selected><?php echo e($proceso_actual->proceso); ?></option>
								<?php endif; ?>
								<?php foreach($procesos as $proceso): ?>
									<?php if($proceso->id!=1): ?>
									<option id="<?php echo e($proceso->id); ?>" value="<?php echo e($proceso->proceso); ?>" ><?php echo e($proceso->proceso); ?></option>
									<?php endif; ?>
								<?php endforeach; ?>
							</select>
						</div>
	...
	<script type="text/javascript">
		...
		if($("#select_meses").val()=="febrero")
			...
			$('#carga_universo').change(function() {
					...
					var url = "<?php echo e(url('/universo_manual')); ?>";
					$.ajax({
						url : url,
						type : "post",
						data : formData,
			...
			{
				//alert($("#mensaje").val());
				window.location.href = "<?php echo e(route('nuevo_universo.create')); ?>?asunto="+$("#asunto").val()+"&mensaje="+$("#mensaje").val()+"&tipo_campania=automatica";
			...
			$.ajax({
				url:"<?php echo e(route('nueva_campana.index')); ?>",
				data:{dato:dato},
