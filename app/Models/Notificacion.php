...
class Notificacion extends Model
{
    ...
    protected function queryNotificacionesExportar($condicion, $request)
    {
        ...
        $q = "select *,
                    ...
                    from destinatarios d
                    left join notificaciones n on (d.notificacion_id = n.id)
                    left join campanias c on (n.campania_id = c.id)
                    left join universos u on (c.universos_id = u.id)
                    left join procesos p on (u.proceso_id = p.id)
                    left join beneficiarios b on (d.rut_beneficiario = b.rut)
                    left join contacto con on (d.rut_beneficiario = con.rut)
